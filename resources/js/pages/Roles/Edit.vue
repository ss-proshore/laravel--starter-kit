        ...
        <div class="lg:col-span-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Edit the basic information for the role.</CardDescription>
            </CardHeader>
            <CardContent>
...
const props = defineProps<{ role: Role, groupedPermissions: PermissionGroup[] }>();
...
const areAllPermissionsSelected = computed<boolean>(() => {
  const allPermissions: Permission[] = props.groupedPermissions.flatMap(group => group.permissions);
  return allPermissions.every((permission: Permission) => form.value.permissions.includes(permission.id));
});
...
const toggleAllPermissions = (): void => {
  ...
  if (areAllPermissionsSelected.value) {
    ...
    form.value.permissions = allPermissions.map((p: Permission) => p.id);
  }
};

// Toggle all permissions in a group
const toggleGroupPermissions = (group: PermissionGroup): void => {
  const allSelected = isGroupSelected(group);
  const permissionIds: number[] = group.permissions.map(p => p.id);
...
function submit(): void {
  router.patch(`/roles/${props.role.id}`, form.value);
}
</script> 